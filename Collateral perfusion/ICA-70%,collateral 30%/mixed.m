...
for T_arterial = 32.3:37.3; %run through different arterial blood temperatures
    ...
    for i = 1:length(temph) % ICA-bf tissue 
        ...
        cbf_w_timeh(i,1) = cbf*(350/100); %convert cbf from ml/100g/min to ml/min for 70% of brain i.e, 350g of brain tissue
        cmr_w_timeh(i,1) = cmr*10^6*(350/100); %convert cmr from mol02/100g/min to umol/min
        dq_w_timeh(i,1) = ((H0-Hb)*cmr - p_blood*c_blood*cbf*(temph(i,1)-T_arterial))*(350/100); %kJ/min
    end
...
%             cmr = normal_cmr;
%         end
        cbf_w_timei(i,1) = 50*(150/100); %convert cbf from ml/100g/min to ml/min
        cmr_w_timei(i,1) = 1.5*10^(-4)*10^6*(150/100); %convert cmr from mol02/100g/min to umol/min
        dq_w_timei(i,1) = ((H0-Hb)*cmr - p_blood*c_blood*cbf*(tempi(i,1)-T_arterial))*(150/100); %kJ/min
    end
...
figure(2)
title({'CBF vs Time for 70% through ICA-bf, i.e.,350g ICA-bf Tissue + 30% collateraly perfused tissue, i.e., 150g Coll-bf Tissue';'Collateraly perfused CBF = 75 ml/min'});
ylabel('CBF (ml/min)');
xlabel('time (min)');
...
figure(3)
title({'CMR vs Time for 70% through ICA-bf, i.e.,350g ICA-bf Tissue + 30% collateraly perfused tissue, i.e., 150g Coll-bf Tissue';'Collateraly perfused CBF = 225umol02/min'});
ylabel('CMR (umol02/min)');
xlabel('min');
legend('Perfusate temperature = 32.3','Perfusate temperature = 33.3', 'Perfusate temperature = 34.3', 'Perfusate temperature = 35.3', 'Perfusate temperature = 36.3', 'Perfusate temperature = 37.3');

figure(4)
title({'Heat Washout (dQ) vs Time for 70% through ICA-bf, i.e.,350g ICA-bf Tissue + 30% collateraly perfused tissue, i.e., 150g Coll-bf Tissue';'Collateraly perfused CBF = 75 ml/min'});
