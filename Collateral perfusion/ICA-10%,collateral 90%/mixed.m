...
for T_arterial = 32.3:37.3; %run through different arterial blood temperatures
    ...
    for i = 1:length(temph) % ICA-bf tissue 
        ...
        cbf_w_timeh(i,1) = cbf*(50/100); %convert cbf from ml/100g/min to ml/min for 10% of brain i.e, 50g of brain tissue
        cmr_w_timeh(i,1) = cmr*10^6*(50/100); %convert cmr from mol02/100g/min to umol/min
        dq_w_timeh(i,1) = ((H0-Hb)*cmr - p_blood*c_blood*cbf*(temph(i,1)-T_arterial))*(50/100); %kJ/min
    end
...
%             cmr = normal_cmr;
%         end
        cbf_w_timei(i,1) = 50*(450/100); %convert cbf from ml/100g/min to ml/min
        cmr_w_timei(i,1) = 1.5*10^(-4)*10^6*(450/100); %convert cmr from mol02/100g/min to umol/min
        dq_w_timei(i,1) = ((H0-Hb)*cmr - p_blood*c_blood*cbf*(tempi(i,1)-T_arterial))*(450/100); %kJ/min
    end
...
figure(2)
title({'CBF vs Time for 10% through ICA-bf, i.e.,50g ICA-bf Tissue + 90% collateraly perfused tissue, i.e., 450g Coll-bf Tissue';'Collateraly perfused CBF = 75 ml/min'});
ylabel('CBF (ml/min)');
xlabel('time (min)');
...
figure(3)
title({'CMR vs Time for 10% through ICA-bf, i.e.,50g ICA-bf Tissue + 90% collateraly perfused tissue, i.e., 450g Coll-bf Tissue';'Collateraly perfused CBF = 225umol02/min'});
ylabel('CMR (umol02/min)');
xlabel('min');
legend('Perfusate temperature = 32.3','Perfusate temperature = 33.3', 'Perfusate temperature = 34.3', 'Perfusate temperature = 35.3', 'Perfusate temperature = 36.3', 'Perfusate temperature = 37.3');

figure(4)
title({'Heat Washout (dQ) vs Time for 10% through ICA-bf, i.e.,50g ICA-bf Tissue + 90% collateraly perfused tissue, i.e., 450g Coll-bf Tissue';'Collateraly perfused CBF = 75 ml/min'});
