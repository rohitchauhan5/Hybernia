...
while ((abs(y_var(length(y_var)) - y_var(i))/(x(length(y_var))-x(i)) > abs((y_var(length(y_var))-y_var(1))/4430)) & (abs(y_var(length(y_var)) - y_var(i))/(x(length(y_var))-x(i))) > 0.0001) %break when average slope, scaled to range, is less than either or these 2 thresholds
%     disp('time')
%     x(i)
%     disp('slope')
